...
(`kw_test`, `comps`) with its own `print` method. `kwtest_def_on`'s
baseline returns a plain list (`statistic`, `df`, `p_value`) with its
own `print` method — neither path shares a class between them.
